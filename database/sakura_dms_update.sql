-- ============================================================================
-- sakura_dms — Skema lengkap (SUDAH DIPERBARUI)
-- ============================================================================
-- Perubahan dari dump sebelumnya (sakura_dms_update.sql):
--   1. users.must_change_password        -> SUDAH ditambahkan (sudah Anda jalankan)
--   2. audit_trail.document_id           -> diubah jadi NULLABLE (Task 1: Log System)
--      supaya aktivitas login, logout, buat/ubah/hapus folder, dan OCR bisa
--      dicatat ke audit_trail meskipun tidak terikat pada satu dokumen.
--
-- File ini HANYA referensi skema final (dokumentasi), BUKAN untuk dieksekusi
-- ulang sebagai CREATE TABLE (tabel-tabel ini sudah ada di database Anda).
-- Kalau Anda hanya butuh MENERAPKAN perubahan yang masih pending, cukup
-- jalankan bagian "MIGRATION YANG PERLU DIJALANKAN" di paling bawah file ini.
-- ============================================================================
...
-- >>> DIPERBARUI: document_id sekarang NULLABLE (sebelumnya NOT NULL) <<<

CREATE TABLE `audit_trail` (
  `id` int NOT NULL AUTO_INCREMENT,
  `document_id` int DEFAULT NULL,                 -- <-- diubah dari "NOT NULL" jadi nullable
...
CREATE TABLE `document_counters` (
  ...
  PRIMARY KEY (`prefix`, `year`)/*T![clustered_index] NONCLUSTERED */
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_general_ci;
...
CREATE TABLE `role_permissions` (
  ...
  `permission_id` int NOT NULL,
  PRIMARY KEY (`role_name`, `permission_id`)/*T![clustered_index] NONCLUSTERED */,
...
-- >>> DIPERBARUI: kolom must_change_password ditambahkan setelah `status` <<<
...
CREATE TABLE `users` (
  ...
  `must_change_password` tinyint(1) NOT NULL DEFAULT '0',   -- <-- kolom baru
...
-- ============================================================================
-- MIGRATION YANG PERLU DIJALANKAN (kalau belum)
-- ============================================================================
-- users.must_change_password sudah Anda tambahkan sebelumnya, jadi baris ini
-- aman dilewati / akan error "Duplicate column" kalau dijalankan ulang.
--
-- ALTER TABLE users
